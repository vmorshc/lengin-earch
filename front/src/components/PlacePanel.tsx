...
type PlacePanelMode = 'create' | 'edit'
  ...
  return (
    <aside
      ...
      <div className="flex items-center justify-between gap-3">
        <h2 className="text-lg font-semibold">{isEditMode ? 'Place details' : 'New place'}</h2>
        <span className="text-xs uppercase tracking-wide text-base-content/60">
          {isEditMode ? 'click outside to save' : 'click outside to close'}
        </span>
      </div>
      ...
      <div className="mt-4 space-y-2 text-sm">
        ...
        <div className="grid grid-cols-2 gap-3 rounded-box bg-base-200/80 p-3 text-xs text-base-content/80">
          <div>
            <div className="uppercase tracking-wide text-base-content/60">Latitude</div>
            <div className="text-base-content">{formatCoordinate(position.lat)}</div>
          </div>
          <div>
            <div className="uppercase tracking-wide text-base-content/60">Longitude</div>
            <div className="text-base-content">{formatCoordinate(position.lng)}</div>
          </div>
        </div>
      </div>

      <label className="mt-4 block text-sm font-medium text-base-content/80" htmlFor="new-place-description">
        Description
      </label>
      <textarea
